	...
	checkUserPass: function (profList, username, password, sess, callback){
		var logged = 0;
			...
			if (username==profList[k].IDENTIFIANT_ADMIN){
				...
				{
					logged++;
					sess.id_professeur = profList[k].IDENTIFIANT_ADMIN;
					sess.nom = profList[k].NOM_ADMIN.toUpperCase();
					sess.prenom = this.firstLetterToUpper(profList[k].PRENOM_ADMIN);
					sess.identifiant = profList[k].IDENTIFIANT_ADMIN;
